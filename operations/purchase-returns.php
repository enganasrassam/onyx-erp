...
if (isset($_GET['delete'])) { $id = (int)$_GET['delete']; $inv = db_fetch_one("SELECT status FROM invoices WHERE id=?",[$id]); if ($inv && $inv['status']==='draft') { db_delete('invoices','id=?',[$id]); flash('success','تم الحذف'); } else flash('error','لا يمكن حذف فاتورة معتمدة'); redirect(APP_URL . '/operations/purchase-returns.php'); }
if (isset($_GET['action'])) { $id=(int)$_GET['id']; $action=$_GET['action']; $sm=['post'=>'posted','cancel'=>'cancelled','mark_paid'=>'paid']; if (isset($sm[$action])) { db_update('invoices',['status'=>$sm[$action]],'id=?',[$id]); db_insert('document_reviews',['doc_type'=>'invoice','doc_id'=>$id,'action'=>$action,'reviewed_by'=>$_SESSION['user_id'],'reviewed_at'=>date('Y-m-d H:i:s')]); flash('success','تم '.($action==='post'?'الترحيل':($action==='cancel'?'الإلغاء':'التحصيل'))); } redirect(APP_URL . '/operations/purchase-returns.php'); }
