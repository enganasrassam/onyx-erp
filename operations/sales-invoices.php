...
if (isset($_GET['delete'])) { $id = (int)$_GET['delete']; $inv = db_fetch_one("SELECT status FROM invoices WHERE id=?",[$id]); if ($inv && $inv['status']==='draft') { db_delete('invoices','id=?',[$id]); flash('success','تم الحذف'); } else flash('error','لا يمكن حذف فاتورة معتمدة'); redirect(APP_URL . '/operations/sales-invoices.php'); }
if (isset($_GET['action'])) { $id=(int)$_GET['id']; $action=$_GET['action']; $sm=['post'=>'posted','cancel'=>'cancelled','mark_paid'=>'paid']; if (isset($sm[$action])) { db_update('invoices',['status'=>$sm[$action]],'id=?',[$id]); db_insert('document_reviews',['doc_type'=>'invoice','doc_id'=>$id,'action'=>$action,'reviewed_by'=>$_SESSION['user_id'],'reviewed_at'=>date('Y-m-d H:i:s')]); flash('success','تم '.($action==='post'?'الترحيل':($action==='cancel'?'الإلغاء':'التحصيل'))); } redirect(APP_URL . '/operations/sales-invoices.php'); }
...
<div class="card mb-4"><div class="card-body d-flex justify-content-between align-items-center">
    ...
    <a href="invoice-form.php?type=sales" class="btn btn-primary d-flex align-items-center gap-1"><svg width="16" height="16" fill="currentColor" viewBox="0 0 20 20"><path d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z"/></svg> إضافة فاتورة</a>
</div></div>
<form method="GET" class="mb-3 d-flex gap-2"><input type="text" name="search" class="form-control" placeholder="بحث..." value="<?= sanitize($search) ?>"><button class="btn btn-outline-secondary">بحث</button><?php if ($search): ?><a href="sales-invoices.php" class="btn btn-outline-secondary">إلغاء</a><?php endif; ?></form>
